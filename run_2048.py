...
def residual_block(data, filters, d_rate,layer_index, bottleneck_factor=0.5, first_dilated_layer=2):
...
with strategy.scope():
    ...
    for i in range(5):
        x = residual_block(x, filters, 3,i)
...
history2=model2.fit(train_dataset, epochs=30, validation_data=val_dataset,callbacks=[es,model_checkpoint_callback])
